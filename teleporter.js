...
shapez.MOD_ITEM_PROCESSOR_HANDLERS.InputTeleporter = function (payload) {
    ...
    payload.entity.components.InputTeleporterComponent.item = payload.items.get(0);
...
class InputTeleporterComponent extends shapez.Component {
    ...
    constructor() {
        ...
        //we need to recreate the map on load 
        let isFound = false;
        if (InputTeleporterComponent.map.has(this.channel)) {
            for (let i = 0; i < InputTeleporterComponent.map.get(this.channel).length; i++){
                if (InputTeleporterComponent.map.get(this.channel)[i] === this) {
                    isFound = true;
                }
            } 
            if (!isFound) {
                InputTeleporterComponent.map.get(this.channel).push(this); 
            }
        } else {
            InputTeleporterComponent.map.set(this.channel,[this])
        }
...
// we define a static map
InputTeleporterComponent.map = new Map();
...
class MetaTeleporterInputBuilding extends shapez.ModMetaBuilding {
    ...
    getIsUnlocked(root) {
        return root.hubGoals.isRewardUnlocked(shapez.enumHubGoalRewards.reward_freeplay);
    }
        ...
        return [
            {
                ...
                description: "This is the input of the teleporter, define a channel et every item this building is given will be outputed by a output teleporter with the same channel.",
        ...
        return [
            {
                ...
                description: "This is the output of the teleporter, it will output the items that are givent to the input with the same channel.",
...
class HUDTeleporterEdit extends shapez.BaseHUDPart {
    ...
    editTeleporterText(entity, type, { deleteOnCancel = true }) {
        ...
        dialog.buttonSignals.ok.add(() => {
            ...
            let oldChannel = teleporterComp.channel;
            // we remove the components from its old channel array
            if (InputTeleporterComponent.map.has(oldChannel)) {
                for (let i = 0; i < InputTeleporterComponent.map.get(oldChannel).length; i++){
                    if (InputTeleporterComponent.map.get(oldChannel)[i] === teleporterComp) {
                        InputTeleporterComponent.map.get(oldChannel).splice(i, 1);
                    }
                } 
            }

            //we add it to its new one
            let newChannel = textInput.getValue();
            if (InputTeleporterComponent.map.has(newChannel)) {
                InputTeleporterComponent.map.get(newChannel).push(teleporterComp);
            } else {
                InputTeleporterComponent.map.set(newChannel, [teleporterComp]);
            }
